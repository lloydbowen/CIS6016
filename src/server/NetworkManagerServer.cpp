...
NetworkManagerServer::NetworkManagerServer() :
	...
	mClientDisconnectTimeout( 30.f ) // 3.0f is probably acceptable - increased to give us time to start client project.
{
}
...
{
	//LOG("DEBUG: %s", "Server is processing a packet!!");
	//LOG("Got a packet from: %s", inFromAddress.ToString().c_str());
...
void NetworkManagerServer::ProcessPacket( ClientProxyPtr inClientProxy, InputMemoryBitStream& inInputStream )
{
	...
	case kHelloCC:
		...
		//LOG("DEBUG: Got a welcome packet from %s", inClientProxy->GetSocketAddress().ToString().c_str() );
		break;
	case kInputCC:
		HandleInputPacket( inClientProxy, inInputStream );
		//LOG("DEBUG: Got a input packet from %s", inClientProxy->GetSocketAddress().ToString().c_str() );
		break;
	default:
...
void NetworkManagerServer::SendOutgoingPackets()
{
	...
	for( auto it = mAddressToClientMap.begin(), end = mAddressToClientMap.end(); it != end; ++it )
	{
        const SocketAddress& inFromAddress = it->first;
        //LOG("Sending packet to: %s", inFromAddress.ToString().c_str());
...
void NetworkManagerServer::CheckForDisconnects()
{
	...
	for( const auto& pair: mAddressToClientMap )
	{
		...
		{
			LOG("DEBUG: Client lost? Time of last packet: %f  \n Last possilble time for packet %f",pair.second->GetLastPacketFromClientTime(), minAllowedLastPacketFromClientTime);
...
void NetworkManagerServer::HandleClientDisconnected( ClientProxyPtr inClientProxy )
{
	...
	if( mAddressToClientMap.empty() )
	{
		...
		LOG("DEBUG: %s", "Last Client Disconnnected - Server Terminating")
	}
}
